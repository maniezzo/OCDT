...
l0:            continue;
         ...
         if(solver.NumVariables() < 1000 && solver.NumConstraints() < 000)
         ...
         {
            using (StreamWriter fout = new StreamWriter($"{dataset}.lp"))
            {  fout.Write("Minimize\n Obj:");
               for(j=0;j<solver.NumVariables();j++)
                  fout.Write($" + 1 x{j}");
               fout.WriteLine();
               fout.WriteLine("Subject to");
               for(i=0;i<solver.NumConstraints();i++)
               {
                  fout.Write($"c{i}: ");
                  for (j = 0; j < lstTableauRows[i].Count; j++)
                     fout.Write($" + 1 x{lstTableauRows[i][j]}");
                  fout.WriteLine(" >= 1");
               }
               fout.WriteLine("Bounds");
               for(j=0;j < solver.NumVariables();j++)
                  fout.WriteLine($" 0 <= x{j} <= 1");
               fout.WriteLine("End");
            }
         }
