...
# reduces AABB given point of different class. Return false if impossible
def reduceAABB(jp,dir):
   for dim in np.arange(ndim):
      ...
      else: # if I find an incompatible presented too late
         for lp in lstPoints:
            ...
            if (X[lp, dim] == X[jp,dim] and X[lp, dim] == minhi[dim]):
               maxhi[dim] = X[lp, dim]
               print(f"Removing {lp} from {lstPoints}")
               lstPoints.remove(lp)  # confliting internal point
         minhi[dim] = minlo[dim] = X[lstPoints[0], dim]
         for lp in lstPoints:
